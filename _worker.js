...
const generateRemark = (index, port) => {
    ...
    switch (index) {
        case 0:
        case 1:
            remark = `☄️ M2rayNG - Domain_${index + 1} : ${port}`;
            break;
        case 2:
        case 3:
            remark = `🚀 M2rayNG - IPv4_${index - 1} : ${port}`;
            break;
        case 4:
        case 5:
            remark = `👑 M2rayNG - IPv6_${index - 3} : ${port}`;
            break;
        default:
            remark = `🐦‍🔥 M2rayNG - Clean IP_${index - 5} : ${port}`;
            break;
    }
...
const buildWorkerLessConfig = async (env, client) => {
    ...
    fragConfig.remarks  = '🕊️ M2rayNG Frag - WorkerLess 🚀'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    bestPing.remarks = '🔥 M2rayNG Frag - Best Ping 💥';
    ...
    bestFragment.remarks = '🏴‍☠️ M2rayNG Frag - Best Fragment 🥷';
...
const getWarpConfigs = async (env, client) => {
    ...
    xrayWarpBestPing.remarks = '🔥 M2rayNG - Warp Best Ping 🚀'
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = `🔥 M2rayNG - WoW ${index/2 + 1} 🇩🇪`;
...
const updateDataset = async (env, Settings) => {
    ...
    const proxySettings = {
        remoteDNS: Settings ? Settings.get('remoteDNS') : currentProxySettings?.remoteDNS || 'https://94.140.15.15/dns-query',
	...
	<body>
		...
		<div class="form-container">
			...
			<form id="configForm">
                ...
                <div class="form-control">
					<label for="nikaNGNoiseMode">😵‍💫 M2rayNG Mode</label>
