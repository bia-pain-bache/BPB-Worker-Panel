            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        if (! await env.asali.get('proxySettings')) await updateDataset(env);
                    ...
                    case '/login':
                        ...
                        let secretKey = await env.asali.get('secretKey');
                        const pwd = await env.asali.get('pwd');
                        if (!pwd) await env.asali.put('pwd', 'admin');

                        if (!secretKey) {
                            secretKey = generateSecretKey();
                            await env.asali.put('secretKey', secretKey);
                        }

                        if (request.method === 'POST') {
                            const password = await request.text();
                            const savedPass = await env.asali.get('pwd');
...
const generateRemark = (index, port) => {
    ...
    switch (index) {
        case 0:
        case 1:
            remark = `🍯 Asali - Domain_${index + 1} : ${port}`;
            break;
        case 2:
        case 3:
            remark = `🍯 Asali - IPv4_${index - 1} : ${port}`;
            break;
        case 4:
        case 5:
            remark = `🍯 Asali - IPv6_${index - 3} : ${port}`;
            break;
        default:
            remark = `🍯 Asali - Clean IP_${index - 5} : ${port}`;
            break;
    }
...
const buildWorkerLessConfig = async (env, client) => {
    ...
    fragConfig.remarks = '🍯 Asali Frag - WorkerLess ⭐'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    if (outProxy) {
        ...
        try {
            ...
            await env.asali.put("proxySettings", JSON.stringify({
                ...proxySettings,
                outProxy: '',
                outProxyParams: ''
            }));
    ...
    bestPing.remarks = '🍯 Asali Frag - Best Ping 💥';
    ...
    bestFragment.remarks = '🍯 Asali Frag - Best Fragment 😎';
...
const getWarpConfigs = async (env, client) => {
    ...
    xrayWoWConfig.remarks = '🍯 Asali - WoW 🌍';
    ...
    warpEndpoints.split(',').forEach((endpoint, index) => {
        ...
        xrayWarpConfig.remarks = `🍯 Asali - Warp ${index + 1} 🇮🇷`;
    ...
    xrayWarpBestPing.remarks = '🍯 Asali - Warp Best Ping 🚀';
...
const updateDataset = async (env, Settings) => {
    ...
    try {
        await env.asali.put("proxySettings", JSON.stringify(proxySettings));
    } catch (error) {
        console.log(error);
...
const Authenticate = async (request, env) => {
    ...
    try {
        const secretKey = await env.asali.get('secretKey');
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}#Asali-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}#Asali-Normal', false)">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=singbox#Asali-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
