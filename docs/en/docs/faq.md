...
??? question "I extracted and used a Proxy IP according to the tutorials, why some sites or applications like X still won't work!"  
...
??? question "Why won't ChatGPT open?"
    Because the panel's default proxy IPs are public and many might appear suspicious to ChatGPT. Set a single clean Proxy IP according to the instructions or enable the `Bypass ChatGPT` option in the routing section of the panel.  
