# :material-cog-outline:{ .md .middle } Xray Fragment settings
...
You can also switch Fragment modes and test based on network situation. Usually default or `Low` mode should be ok. Please note that by switching to `Medium`, `High` or `Severe` profiles, handshake ping goes higher but it can result in better and more stable connection in high situations. The `Smart Fragment` config is always the best and smart solution, just connect and wait for at least 30 seconds.
...
!!! tip
    If you cannot find the optimum fragment settings for your ISP, there is a **Smart Fragment** configuration available in the subscription. Simply connect to it and wait a short while; it tests nearly all valid fragment settings and automatically connects to the best one.

!!! note
    Fragment values have maximum limits. The Length cannot exceed 500, and the Delay cannot exceed 30ms.
